...
import Interest from "./FormSteps/Interest";
import References from "./FormSteps/References";

import NavigationButtonWrapper from "./NavigationButtonWrapper";

function Extra({ formData, resumeID }: any) {
  ...
  const handleFieldUpdate = async (name: any, value: string) => {
    ...
    const path = `resumeData.formData.Extras`;
  ...
  const onSubmit = async (data: any) => {
    console.log(data);
    dispatch(nextStep());
    const path = `resumeData.formData.completeForm`;
    dispatch(onResumeInput({ path, value: true }));
    ...
    try {
      ...
      await dispatch(
        updateResumeOnline({
          _id: resumeID,
          value: true,
          path: `resumeData.formData.completeForm`,
        })
      );
    } catch (error) {
